                               ...
                               float age, float normalizedAge)
{
    ...
    return float4(position, 1.0f); //mul(mul(float4(position, 1), View), Projection);
}
...
VertexShaderOutput VertexShader(VertexShaderInput input)
{
    ...
    float4 pos = ComputeParticlePosition(input.Position, input.Velocity,
                                              age, normalizedAge);
...
PS_MRT_Output NonRotatingPixelShader(NonRotatingPixelShaderInput input)
{
...
// point sprite itself.
PS_MRT_Output RotatingPixelShader(RotatingPixelShaderInput input)
{
	PS_MRT_Output output;
